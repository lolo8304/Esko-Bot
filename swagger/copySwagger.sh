...
do
	...
	then
	  ...
	  echo "`date` - copy swagger.local.json file to git for localhost"
		cat $DLOC/swagger.json | sed 's/esko-bot.herokuapp.com/localhost\:3978/g' | sed 's/https:\/\/localhost/http:\/\/localhost/g' > $DLOC/swagger.local.json
	fi
	...
	then
		...
		cat $DLOC/swagger.yaml | sed 's/esko-bot.herokuapp.com/localhost\:3978/g' | sed 's/https:\/\/localhost/http:\/\/localhost/g' > $DLOC/swagger.local.yaml
	fi
	sleep 1
done
